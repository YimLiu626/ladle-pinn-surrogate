...
\title{A VOF-Guided Neural Surrogate for Position-Conditioned Flow Prediction and Operating-Parameter Evaluation in Argon-Stirred Ladle Refining}
...
Training: center-blowing $Q=40,60,80$ NL/min. Extrapolation testing: center-blowing $Q=100,120$ NL/min. Cross-position testing: eccentric-blowing $Q=40,80,120$ NL/min ($m=1$). Several representative baselines were evaluated: POD-ROM, MLP $8\times384$, MLP $6\times128$, and DeepONet ($p=50$). Kriging GPR and KAN were also attempted but encountered scalability and implementation barriers respectively; these are discussed in Section 6. Metrics: $R^{2}$ on velocity magnitude, per-field $R^{2}$, multi-seed ensemble stability.
