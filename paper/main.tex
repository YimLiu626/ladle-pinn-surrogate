...
\title{\bfseries A VOF-Guided Neural Surrogate for Position-Conditioned Flow Prediction in Argon-Stirred Ladle Refining}

\author[1]{Yiming Liu}
\author[1]{Wentao Zhang}
\author[1]{Xutong Wang}
\author[1]{Guangxin Wu\thanks{Corresponding author. E-mail: gxwu@shu.edu.cn}}
\affil[1]{School of Materials Science and Engineering, State Key Laboratory of Advanced Special Steel, Shanghai University, Shanghai 200444, PR China}
...
\bigskip
\noindent\textbf{Keywords:} VOF-filtered neural surrogate; Ladle furnace; Position-conditioned prediction; POD-ROM; CFD-informed machine learning.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig2_VOF_stratification.png}
...
Three-phase stratification is identical across all five $Q$ values (Fig.~\ref{fig:vof}), confirming consistent CFD setup. Centerline velocity profiles (Fig.~\ref{fig:centerline}) show close agreement for all eight configurations (center-blowing panels a,c; eccentric-blowing panels b,d); relative errors remain below 10\% through most of the domain.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig3_centerline.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig4_ablation_baseline.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig6_POD_vs_PINN.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig7_scatter.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig8_3D_error.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig9_performance.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig10_convergence.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../pic/Fig11_3D_plume.png}
...
The benefit of VOF filtering is twofold. First, it removes gas- and slag-dominated cells where mixture properties vary sharply, avoiding the severe ill-conditioning that would affect a full-domain surrogate. Second, the majority of retained cells belong to the steel-dominated bulk region where the flow approaches single-phase behavior. The improvement is therefore not from a strictly pure-steel mask but from a physically motivated restriction of the learning domain ($+0.344$ $R^{2}$ over hard thresholding; full-domain training collapses).

POD-ROM achieves the best center-blowing accuracy ($R^{2}=0.994$) with only two spatial modes. This is not an indictment of the MLP but a reflection of problem structure: three snapshots yield two modes capturing all variance, and RBF interpolation of smoothly varying coefficients yields near-perfect prediction. The MLP learns a general 5D$\to$4D mapping from a three-point sampling of the $Q$ dimension---a fundamentally harder task. That it approaches POD-level accuracy ($R^{2}=0.983$) demonstrates the effectiveness of the neural network approach.

However, the center-trained POD does not transfer to eccentric blowing because its modes capture a center-symmetric plume structure. Parametric, local, or clustered POD variants may alleviate this limitation with additional snapshots per configuration. The MLP's $m$ parameterization provides a simpler alternative: a single network serves both configurations. This trade-off---single-configuration optimality versus multi-configuration flexibility---defines the practical value proposition of the proposed approach.

Regarding physics-informed regularization, the continuity constraint provides a consistent but small gain ($+0.014$ $R^{2}$) for the $6\times128$ model, where limited capacity benefits from additional guidance. For the $8\times384$ model, pure data supervision is sufficient. The momentum residual does not help because the DPM bubble buoyancy---the actual plume driver---is absent from the homogeneous RANS formulation. BC constraints are unnecessary; no-slip is implicitly encoded in the CFD data. These findings suggest that for CFD-informed surrogate problems with dense, noise-free training data, physics regularization offers diminishing returns as model capacity increases.
